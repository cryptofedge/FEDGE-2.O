# FEDGE 2.O â€” Generational Wealth Ecosystem

> **Built with purpose. Powered by NYC energy. Designed for the culture.**
> The AI brain behind a full suite of financial education games and tools â€” built by **Eclat Universe**, powered by **Rafael Fellito Rodriguez Jr.**

FEDGE 2.O is an intelligent AI agent ecosystem designed to put generational wealth in the hands of people who were never taught how to build it. Every game, every tool, and every interaction is powered by this core â€” running on Eclat Universe - Rafael Fellito Rodriguez with a 9-agent architecture built for scale.
...
| [lock-in-game](https://github.com/cryptofedge/lock-in-game) | Mental health and discipline game â€” build focus, habits, and consistency |
| [world-stage](https://github.com/cryptofedge/world-stage) | Music industry RPG â€” travel the world, build your career, reach the top |
...
| [Eclatcrypto](https://github.com/cryptofedge/Eclatcrypto) | $ECLT â€” Solana token. Pure community, pure vibes |

---

## Agent Architecture

FEDGE 2.O runs on a **9-agent AI system** powered by Eclat Universe - Rafael Fellito Rodriguez. The lead agent delegates tasks to 8 specialist subagents â€” one per game â€” running in parallel on a shared filesystem.

```
FEDGE 2.O LEAD AGENT  â€”  SOUL.md Â· MEMORY.md Â· USER.md
|
|-- credit-agent          ->  fedge-2-credit-game
|-- trustfund-agent       ->  fedge2-trust-fund-game
|-- mentalhealth-agent    ->  Mental Health Game (co-built with Alberto)
|-- lockin-agent          ->  lock-in-game
|-- worldstage-agent      ->  world-stage
|-- genwealth-agent       ->  generational-wealth-game
|-- tradestreet-agent     ->  tradestreet-fedge
|-- hustle-agent          ->  Hustle-Politics-
```

Each agent has its own model, system prompt, tools, and persistent memory. Nothing is forgotten between sessions.
...
| `MEMORY.md` | Long-term memory â€” curated across all sessions |
...
| `HEARTBEAT.md` | Proactive check-in and background task config |
| `TOOLS.md` | Tool registry and configuration |
| `agent/shared/` | Shared filesystem for all subagents to communicate |
...
- **Primary Color:** `#E87012` (Eclat Orange)
- **Font:** IBM Plex Mono + IBM Plex Sans
- **Theme:** Dark (`#06060F` base)
- **Identity:** NYC energy. Built for the culture. Powered by purpose.
...
```bash
git clone https://github.com/cryptofedge/FEDGE-2.O.git
cd FEDGE-2.O
npm install
```
...
```bash
cp .env .env.local
# Add your Anthropic API key and any other credentials
```
...
1. **SOUL.md** â€” Identity, values, NYC personality, hard limits
2. **MEMORY.md** â€” Long-term curated memory from past sessions
3. **USER.md** â€” Fellito's context, preferences, and goals
4. **AGENTS.md** â€” Orchestration manifest for all 8 subagents
5. **Skills** â€” 54 domain-specific skill modules loaded on demand

The agent powers:

- **Financial Education** â€” Credit, investing, trust funds, generational wealth
- **Game Intelligence** â€” AI brain for all 8 FEDGE 2.O games
- **Trading Advisor** â€” Portfolio coaching via TradeStreet
- **Mental Health Support** â€” Safe, trauma-informed guidance via Lock-In
- **Music Industry** â€” Artist development coaching via World Stage
- **Crypto Guidance** â€” Solana ecosystem support via Eclatcrypto

---

## Roadmap

- [x] Core agent â€” SOUL, MEMORY, USER, AGENTS wired
...
- [ ] Dreaming â€” automated memory refinement every 72 hours
- [ ] Outcomes â€” persistent session memory per subagent
